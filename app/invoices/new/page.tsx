...
export default function NewInvoicePage() {
  ...
  // Pricing — stored as per-unit rates; totals are derived
  ...
  const sellingPerUnit = parseFloat(sellingPricePerUnit) || 0;
  const purchasePerUnit = parseFloat(purchasePricePerUnit) || 0;
  const sellingAmount = qty * sellingPerUnit;
  const purchaseAmount = qty * purchasePerUnit;
  const vat = vatEnabled ? (sellingAmount * (parseFloat(vatRate) || 0)) / 100 : 0;
  const total = sellingAmount + vat;
  const grossProfit = sellingAmount - purchaseAmount;
  const grossMargin = sellingAmount > 0 ? (grossProfit / sellingAmount) * 100 : 0;

  const sym = currency === "KES" ? "KSh" : "USD";
  const fmt = (n: number) =>
    `${sym} ${n.toLocaleString("en-KE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <section className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className={labelCls}>Selling Price / {unit} *</label>
                <input
                  required
                  type="number"
                  step="0.01"
                ...
                />
                {qty > 0 && sellingPerUnit > 0 && (
                  <p className="text-xs text-gray-500 mt-1">
                    {qty} {unit} × {sym} {sellingPerUnit.toLocaleString()} = <span className="font-semibold text-gray-800">{fmt(sellingAmount)}</span>
                  </p>
                )}
              </div>

              <div>
                <label className={labelCls}>Purchase Price / {unit} <span className="text-gray-400 font-normal">(internal)</span></label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
                ...
                />
                {qty > 0 && purchasePerUnit > 0 && (
                  <p className="text-xs text-gray-500 mt-1">
                    {qty} {unit} × {sym} {purchasePerUnit.toLocaleString()} = <span className="font-semibold text-gray-800">{fmt(purchaseAmount)}</span>
                  </p>
                )}
              </div>
                ...
                {vatEnabled && (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-20 border border-gray-200 rounded-lg px-2 py-1.5 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
            ...
            {(sellingAmount > 0 || purchaseAmount > 0) && (
              <div className="mt-5 pt-4 border-t border-gray-100 space-y-2 text-sm">
                ...
                {purchaseAmount > 0 && (
                  <div className={`flex justify-between text-xs pt-1 border-t border-dashed border-gray-100 font-medium ${grossProfit >= 0 ? "text-green-600" : "text-red-600"}`}>
                    <span>Gross Profit ({grossMargin.toFixed(1)}%)</span>
          ...
          <div className="flex gap-3 justify-end">
            <Link href="/" className="px-4 py-2 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors bg-white">
              Cancel
            </Link>
            <button
              type="submit"
